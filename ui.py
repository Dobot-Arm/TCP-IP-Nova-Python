...
class RobotUI(object):
    ...
    def __init__(self):
        ...
        self.label_dash = Label(self.frame_robot, text="Dashboard/Move Port:")
        ...
        self.entry_dash.place(rely=0.2, x=360)
        ...
        # Control Function
        self.frame_control = LabelFrame(self.frame_dashboard, text="控制器IO",
                                          labelanchor="nw", bg="#FFFFFF", pady=10, width=430, height=60, border=2)

        self.frame_control.place(relx=0, rely=0.4, relheight=0.6)

        # Enable/Disable
        self.button_enable = self.set_button(master=self.frame_dashboard,
                                             text="Enable", rely=0.001, x=10, command=self.enable)
        self.button_enable["width"] = 7
        self.global_state["enable"] = False

        # Stop Robot / Clear Error
        self.set_button(master=self.frame_dashboard,
                        text="Stop Robot", rely=0.001, x=145, command=self.reset_robot)
        self.set_button(master=self.frame_dashboard,
                        text="Clear Error", rely=0.001, x=290, command=self.clear_error)

        # Speed Ratio
        self.label_speed = Label(self.frame_dashboard, text="Speed Ratio:")
        self.label_speed.place(rely=0.001, x=430)

        s_value = StringVar(self.root, value="50")
        self.entry_speed = Entry(self.frame_dashboard,
                                 width=6, textvariable=s_value)
        self.entry_speed.place(rely=0.001, x=520)
        self.label_cent = Label(self.frame_dashboard, text="%")
        self.label_cent.place(rely=0.001, x=550)

        self.set_button(master=self.frame_dashboard,
                        text="Confirm", rely=0.001, x=586, command=self.confirm_speed)



        # Terminal Function
        self.frame_control = LabelFrame(self.frame_dashboard, text="末端IO",
                                          labelanchor="nw", bg="#FFFFFF", pady=10, width=430, height=60, border=2)

        self.frame_control.place(relx=0.5, rely=0.4, relheight=0.6)



        # TerminalDO:Digital Outputs
        self.label_terminalDigitial = Label(
            self.frame_dashboard, text="Digital Outputs: Index:")
        self.label_terminalDigitial.place(rely=0.65, relx=0.5, x=10)

        i_value = IntVar(self.root, value="1")
        self.entry_terminalIndex = Entry(
            self.frame_dashboard, width=5, textvariable=i_value)
        self.entry_terminalIndex.place(rely=0.65,relx=0.5, x=160)

        self.label_terminalStatus = Label(self.frame_dashboard, text="Status:")
        self.label_terminalStatus.place(rely=0.65,relx=0.5, x=220)

        self.combo_terminalStatus = ttk.Combobox(self.frame_dashboard, width=5)
        self.combo_terminalStatus["value"] = ("On", "Off")
        self.combo_terminalStatus.current(0)
        self.combo_terminalStatus["state"] = "readonly"
        self.combo_terminalStatus.place(rely=0.65,relx=0.5, x=275)

        self.set_button(self.frame_dashboard, "Confirm",
                        rely=0.6, x=780, command=self.confirm_terminal_do)
        ...
        self.label_digitial = Label(
            self.frame_dashboard, text="Digital Outputs: Index:")
        self.label_digitial.place(rely=0.65, x=10)

        i_value = IntVar(self.root, value="1")
        self.entry_index = Entry(
            self.frame_dashboard, width=5, textvariable=i_value)
        self.entry_index.place(rely=0.65, x=160)

        self.label_status = Label(self.frame_dashboard, text="Status:")
        self.label_status.place(rely=0.65, x=220)
        ...
        self.combo_status.place(rely=0.65, x=275)

        self.set_button(self.frame_dashboard, "Confirm",
                        rely=0.6, x=350, command=self.confirm_do)
    ...
    def move_jog(self, text):
        if self.global_state["connect"]:
            self.client_dash_move.MoveJog(text)

    def move_stop(self, event):
        if self.global_state["connect"]:
            self.client_dash_move.MoveJog("")
    ...
    def connect_port(self):
        if self.global_state["connect"]:
            ...
            try:
                print("连接成功")
                self.client_dash_move = DobotApiDashboardAndMove(
                    self.entry_ip.get(), int(self.entry_dash.get()), self.text_log)
                # self.client_dash_move = DobotApiMove(
                #     self.entry_ip.get(), int(self.entry_dash.get()), self.text_log)
    ...
    def enable(self):
        if self.global_state["enable"]:
            self.client_dash_move.DisableRobot()
            self.button_enable["text"] = "Enable"
        else:
            self.client_dash_move.EnableRobot()
            # if need time sleep
            # time.sleep(0.5)
    ...
    def reset_robot(self):
        self.client_dash_move.StopRobot()

    def clear_error(self):
        self.client_dash_move.ClearError()

    def confirm_speed(self):
        self.client_dash_move.SpeedFactor(int(self.entry_speed.get()))

    def movj(self):
        self.client_dash_move.MovJ(float(self.entry_dict["X:"].get()), float(self.entry_dict["Y:"].get()), float(self.entry_dict["Z:"].get()),
    ...
    def joint_movj(self):
        self.client_dash_move.JointMovJ(float(self.entry_dict["J1:"].get()), float(self.entry_dict["J2:"].get()), float(self.entry_dict["J3:"].get()),
                                   float(self.entry_dict["J4:"].get()), float(self.entry_dict["J5:"].get()), float(self.entry_dict["J6:"].get()))

    def confirm_do(self):
        if self.combo_status.get() == "On":
            print("高电平")
            self.client_dash_move.DO(int(self.entry_index.get()), 1)
        else:
            print("低电平")
            self.client_dash_move.DO(int(self.entry_index.get()), 0)

    def confirm_terminal_do(self):
        if self.combo_terminalStatus.get() == "On":
            print("末端 高电平")
            self.client_dash_move.ToolDO(int(self.entry_terminalIndex.get()),1)
        else:
            print("末端 低电平")
            self.client_dash_move.ToolDO(int(self.entry_terminalIndex.get()),0)
    ...
    def display_error_info(self):
        error_list = self.client_dash_move.GetErrorID().split("{")[1].split("}")[0]
